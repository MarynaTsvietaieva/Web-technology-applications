...
export default class articleFormsHandler {
    ...
    assignFormAndArticle(formElementId, cssClass2hideElement,articleId,offset,totalCount,ifMy){
    ...
    processArtNewFrmData(event){
        ...
        fetch(`${this.serverUrl}/article`, postReqSettings)
            ...
            .then(responseJSON => {
                ...
                $.getJSON(`https://wt.kpi.fei.tuke.sk/api/article/?max=10&offset=${ this.totalCount}`, callbackFuncWithData);
                console.log(`https://wt.kpi.fei.tuke.sk/api/article/?max=10&offset=${this.totalCount}`);
                function callbackFuncWithData(json) {
                    data.id =  json.articles[json.articles.length-1].id;
                }

                var ajax = new XMLHttpRequest();
                ajax.open("PUT",`https://wt.kpi.fei.tuke.sk/api/article/${data.id}/tag/myVinoTag` , true);
                ajax.send();
            })
            .catch(error => {
            ...
            .finally(() => window.location.hash = `#articles/${this.offset}/${ this.totalCount}/${this.ifMy}`);
    ...
    processArtEditFrmData(event) {
        ...
        $.getJSON(`https://wt.kpi.fei.tuke.sk/api/article/${this.articleId}`, callbackFuncWithData);
        function callbackFuncWithData(json) {
            if(`${json.tags}`.split(",").includes("myVinoTag")){
                articleData.tag = true;
            }
        }
        ...
        fetch(`${this.serverUrl}/article/${this.articleId}`, postReqSettings)
            ...
            .then(responseJSON => {
                ...
                if(articleData.tag){
                    var ajax = new XMLHttpRequest();
                    ajax.open("PUT",`https://wt.kpi.fei.tuke.sk/api/article/${this.articleId}/tag/myVinoTag` , true);
                    ajax.send();
                }
            })
            .catch(error => {
            ...
            .finally(() => window.location.hash = `#article/${this.articleId}/${ this.offset}/${this.totalCount}/0/${this.ifMy}`);
